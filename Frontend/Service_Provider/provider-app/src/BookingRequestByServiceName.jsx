...
const BookingRequestByServiceName = () => {
  const [services, setServices] = useState([]);
  const [error, setError] = useState(null);

  const getAllServices = async (providerid, serviceid) => {
    try {
      setError(null);
      const response = await fetch(`${SERVER}/bookedService/getBookedByServiceAndProvider/${providerid}/${serviceid}`);
  ...
  useEffect(() => {
    ...
    if (providerid && serviceid) {
      getAllServices(providerid, serviceid);
    } else {
...
export default BookingRequestByServiceName;
